...
def apply_tool_instructions(system_prompt: str, should_generate_images: bool) -> str:
    ...
    stripped_tool_instructions = TOOL_USE_INSTRUCTIONS.strip()

    if stripped_tool_instructions in stripped_system_prompt:
        return stripped_system_prompt + f"\n\nImage generation is {image_flag}."

    return (
        stripped_system_prompt
        + "\n\n"
        + stripped_tool_instructions
        + f"\n\nImage generation is {image_flag}."
    )
