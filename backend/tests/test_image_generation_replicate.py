...
@pytest.mark.asyncio
async def test_call_replicate_uses_default_z_image_model(
    ...
    assert captured["model_path"] == replicate.Z_IMAGE_TURBO_MODEL_PATH
    assert captured["api_token"] == "token-123"


@pytest.mark.asyncio
async def test_call_replicate_can_use_flux_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, object] = {}
    ...
    result = await replicate.call_replicate(
        {"prompt": "test", "seed": 1}, "token-123", model="flux_2_klein"
    )
