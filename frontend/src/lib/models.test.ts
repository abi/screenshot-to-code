...
const GPT_XHIGH = CodeGenerationModel.GPT_5_5_XHIGH;
const PRO_LOW = CodeGenerationModel.GEMINI_3_1_PRO_PREVIEW_LOW;

describe("getVariantLabel", () => {
  test("image/create: Flash-minimal -> Fast; Pro-high & GPT-5.5 xhigh -> Max", () => {
    ...
    expect(getVariantLabel(GPT_XHIGH, ctx)).toEqual({ text: "Max", tone: "max" });
  ...
  test("text/create: Flash-minimal -> Fast, GPT-5.5 xhigh -> Max; Pro-low unlabelled", () => {
  ...
  test("image/update: no labels (heavyweight variants reused but unlabelled)", () => {
    ...
    expect(getVariantLabel(GPT_XHIGH, ctx)).toBeNull();
  });
